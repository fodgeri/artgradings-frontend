...
Postgres + Auth, accessed through `@supabase/ssr`. Development is **local-first**:
a full stack runs in Docker (`npm run db:start`), and migrations reach the hosted
EU **dev** project by `npm run db:push` after merge. Spec:
`docs/superpowers/specs/2026-08-18-supabase-foundation-design.md`.

| File | Role |
|---|---|
| `lib/supabase/env.ts` | The only place the three environment variables are read, and the only place that throws |
| `lib/supabase/client.ts` | Browser client, for Client Components |
| `lib/supabase/server.ts` | Server Components, Server Actions, Route Handlers — async, per request |
| `lib/supabase/proxy.ts` | `updateSession` — token rotation only, edge-safe |
| `lib/supabase/admin.ts` | `service_role` client; bypasses every policy, `server-only` |
| `lib/supabase/database.types.ts` | Generated by `npm run db:types`; committed, never hand-edited |
| `supabase/migrations/` | Hand-written, replayed from empty by CI |
| `supabase/seed.sql` | Local-only sample data |
| `supabase/tests/` | pgTAP; `000-setup.sql` holds the impersonation helpers |

```bash
npm run db:start    # local stack (Studio on :54323, Mailpit on :54324)
npm run db:reset    # replay every migration into an empty DB, then the seed
npm run db:test     # the pgTAP suite
npm run db:new x    # create a migration
npm run db:types    # regenerate database.types.ts from the LOCAL schema
npm run db:push     # apply migrations to the hosted dev project — after merge
```
...
- **Nothing may run between `createServerClient` and `getClaims()`** in
  ...
  inherits neither. Omitting the cookies logs users out only on requests that
  redirect; omitting the headers lets a CDN cache a response carrying a session
  token. Copy only those headers: the whole set would drag Next's internal
  `x-middleware-*` markers onto a redirect and break it.
...
definition. Everything else — `SUPABASE_SECRET_KEY`, `R2_*`,
